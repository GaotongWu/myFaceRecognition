...
MODEL =load_model('my_model.h5')  # _______
...
def classify(path_to_image):
    ...
    img =cv2.imread(path_to_image,1)  # ______
    ...
    predictions = MODEL.predict(image_to_be_classified) # ______
    ...
    # Hint: np.argmax
    label = np.argmax(predicitions) # ________
    ...
    # Hint: np.sum()
    label_value = label[i] # _______
    total = np.sum(label) # _________
